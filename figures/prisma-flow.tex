% Search Methodology Diagram (replaces prior PRISMA flow diagram)
% SoK: Client-Side Anti-Automation After the VLM
% Compiled with: pdflatex or lualatex
% Requires: tikz package
% NOTE: This diagram documents the search methodology. It does not follow the PRISMA 
% structured eligibility format, which was designed for clinical meta-analyses (see Section 3.1).
...
\node[excluded, right=0.8cm of identify, yshift=-0.3cm] (excl-irrelevant) {
    Out-of-scope domain: \textbf{n = 55}
};
...
\node[font=\footnotesize, text width=8cm, align=left, below=1.0cm of categories, text=black!50] (criteria) {
...
\node[font=\footnotesize\itshape, text width=8cm, align=left, below=1.8cm of categories, text=black!40] (method-note) {
    This diagram documents the search process transparently. The PRISMA structured \\
    eligibility format is not applied here, as it was designed for clinical meta-analyses; \\
    see Section 3.1 for methodology details.
};
