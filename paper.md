...
Previous efforts to systematize this space — following the SoK tradition established across security subfields [1, 2, 3] — have attempted to build unified taxonomies that simultaneously classify all existing defenses and prescribe future directions. This paper argues that such an approach faces a structural challenge because the 2024–2025 emergence of production-capable Vision-Language Models (GPT-4o, Claude Computer Use, Gemini 2.0) and agentic AI frameworks (WebVoyager, GPT-4o's Computer Use Agent (CUA), Operator) has introduced a qualitatively distinct attack vector — "Operator Synthesis" — that bypasses the core detection premise of probabilistic client-side defenses for the Advanced Persistent Bot (APB) threat model, representing the most severe escalation within the Environmental Forgery paradigm rather than a clean categorical break.
...
**C1 — A Two-Part SoK Structure (Part I, Section 3; Part II, Sections 4–6).** We explicitly partition the analysis into a historical retrospective (Part I, Section 3) and a forward-looking analysis under the Operator Synthesis paradigm (Part II, Sections 4–6). This resolves the structural dissonance that arises when a single taxonomy attempts to simultaneously classify active defenses and acknowledge their obsolescence.
...
|---|---|---|
| **Scraping / Resource Exhaustion** | Quadrant I: Point-in-Time VM, Behavioral Biometrics, Compute-Bound Challenges (auxiliary) [66] | Quadrant III: Session-bound rate limiting, quota enforcement |
| **Account Takeover / Fraud** | Quadrant II: Stateful Telemetry (login risk scoring), Platform Anonymous Attestation (PATs) [66] | Quadrant IV: Hardware-Anchored Determinism (DBSC, Passkeys, WebAuthn) [65] |

Table: The Axis A/B threat-model quadrant matrix in NIST 800-63 terminology [65, 66].
...
**2005–2012: The CAPTCHA Era.** Text-distortion, image-recognition, and audio CAPTCHAs tested human perceptual ability [68, 70]. OCR and CNN-based solvers progressively eroded CAPTCHA effectiveness. CAPTCHA-solving farms emerged as economic bypass mechanisms (∼$1 per 1,000 solved CAPTCHAs) [51]. By 2014, automated solvers achieved 99.8% accuracy on reCAPTCHA challenges [70]. Visual CAPTCHAs remain in production as fallback escalation paths (Arkose Labs, hCaptcha), but ceased to be the standalone frontier of defense.
...
Anderson and Moore [46] established information security as fundamentally an economic problem, a line subsequently extended to cybersecurity policy [48] and to the macro-level measurement of cybercrime costs [50]. Herley and Florêncio [47] demonstrated that underground economy pricing follows predictable patterns — "nobody sells gold for the price of silver." Applied to client-side anti-automation, these frameworks yield the critical insight that client-side defenses operate under a *forgery model* rather than a *cryptanalysis model*: the attacker's cost is determined by market prices (proxy IPs, human labor, GPU compute, malware infections), not by a mathematical security parameter.
...
**Deduplication and Screening.** Initial search across six databases yielded approximately 230 unique items. Title and abstract screening against inclusion/exclusion criteria reduced the corpus to 95 publications. Full-text architectural assessment eliminated 17 publications that lacked sufficient mechanism-level documentation for taxonomic classification, leaving the final corpus of 78 publications, subsequently expanded to 88 references through additions for agentic web-navigation frameworks, SDK-proxy network research, the Private Access Control Tokens (PACT) initiative, and 2025–2026 VLM-based CAPTCHA measurement work.
...
**Source Corpus.** 88 references: 58 academic papers, 10 standards and RFCs, and 20 grey-literature items (vendor whitepapers, threat-intelligence reports, preprints, and blog posts).
...
Table: Cost-type shift per L1–L4 layer under Operator Synthesis. EF = Environmental Forgery; OS = Operator Synthesis.
...
The temporal arms race between AST obfuscators and symbolic execution engines — documented in prior work [24] — pitted compile rotation (`T_Life`) against the attacker's reverse-engineering pipeline (`T_RE`) measured via automated tools (Syntia [26], DOBF [28]), building on earlier symbolic execution for JavaScript [25], deobfuscation of virtualization-obfuscated code [30], and program-property prediction over obfuscated binaries [29]. The tipping point `T_RE < T_Life` was approaching for the best-defended VMs. Under Operator Synthesis, this race terminates at the VM level: the attacker never inspects the defensive bytecode, black-boxing the obfuscation layer entirely. The wider lesson: software-only obfuscation has a structural ceiling — it cannot impose cost against an adversary who bypasses the inspection step. However, application-level workflow RE (Section 5.5) restores a distinct, cheaper temporal constraint at the DOM layer.
...
The tiers below measure resilience specifically against Operator Synthesis — that is, whether the detection mechanism is input-modality-dependent. They do not measure total defense strength: Type IV and Type V face additional attack chains (device compromise, SDK proxying) that are independent of Operator Synthesis and whose economics are analyzed separately in Section 4.3. The tier labels are mutually exclusive by construction: Tier 2 denotes degradation in which a residual detection surface survives in weakened form, while Tier 3 denotes operational substitution, in which a collapsed constraint is replaced by a substitute cost rather than eliminated outright. No mechanism occupies more than one tier.

| Tier | Definition | Types | Degradation Mechanism |
|------|------------|-------|-----------------------|
| 1 | Input-modality-independent, hardware-anchored — structurally resilient against Operator Synthesis | Type IV (PATs, hardware-anchored), Type V (DBSC/Passkeys) | Cryptographic binding independent of input modality; VLM cannot forge attestation keys. Device compromise and SDK-proxying are orthogonal attack chains (Section 4.3). |
...
| 3 | Operationally substituted — structural constraint lifted, replaced by orchestration cost | Type II (Stateful Telemetry) | Profile-aging latency constraint lifted; state-orchestration complexity and infrastructure cost substitute for the anti-detect browser license |
...
**Published list pricing (as of Q1 2026).** List pricing for frontier VLMs:
...
**Supply elasticity.** Residential proxy markets exhibit a tiered supply structure [76], and the elasticity language below follows standard microeconomic definitions [49]. The price bands below reflect retail list pricing published by commercial residential-proxy providers as of Q1 2026; they are indicative of market structure rather than precise transaction prices, which vary with volume commitments, network quality, and provider-specific discounting:
...
Prior work correctly identified human labor — CAPTCHA-solving farms at ∼$1 per 1,000 challenges [51], click-farms and freelance labor markets for behavioral challenges [52] — as the global cost floor for bypassing behavioral biometrics under Environmental Forgery. Under Operator Synthesis, this floor shifts:
...
Under Environmental Forgery, the temporal arms race between defender compile rotation and attacker RE was quantified as a race condition (Section 3.5):
...
Industry deployment over 2024–2026 has advanced along three tracks — hardware-anchored session determinism (DBSC), credential-phishing-resistant authentication (passkeys), and anonymous attestation (Privacy Pass, Apple PATs, and the 2026 PACT initiative). Each track relocates trust into vendor-controlled infrastructure, and the shared structural consequence is the thesis of this section: the Anonymous Authentication Gap is progressively converted into an Attestation Market Centralization problem (§6.3), which the PACT software-anchor turn extends rather than resolves (§6.4).

### 6.1 DBSC and the Session-Hijacking Threat Model

DBSC [36, 37, 61] cryptographically binds session cookies to a device-resident key (TPM/Secure Enclave), with periodic proof-of-possession [75] throughout the session lifetime. This targets the session-hijacking threat model (NIST 800-63 Authenticated/ATO quadrant [65]) and is structurally inapplicable to anonymous traffic.

The infostealer economy [53, 54, 55, 56] provides the economic bypass, and automated black-box auditing of web authentication weaknesses [57] has industrialised the discovery of session-handling flaws: malware infections export both the session cookie and the device-bound key simultaneously from a compromised device. At $30–$200/month for botnet access (with dark-web market prices documented up to $4,800/month for premium botnets), the economic ceiling is the black-market infection cost, not the cryptographic protocol strength.
...
Passkeys (FIDO2/WebAuthn [31]) eliminate shared secrets and credential phishing through hardware-enrolled cryptographic authentication [71] — the latest step in the documented evolution of password-based authentication [69]. Kuchhal et al. [32] found only 4.4% of authenticators carry Level 2+ certification offering malware resistance. Tarrach et al. [33] identified message integrity gaps accessible to browser extensions, and timing side channels in authenticator behavior [34] further erode the assumption of unobservable key operations. The structural limit is the practical difficulty of guaranteeing key-storage integrity across the diversity of consumer devices and cloud-synchronized passkey stores [35].
...
Initial empirical measurements have begun to appear: generalized VLM agents now solve production CAPTCHAs with reported success rates that make large-scale operation economically plausible [86], CAPTCHA-class benchmarks against VLM attacks are being standardized [87], and reasoning-action training pipelines for GUI-agent CAPTCHA solving are being industrialized [88]. What remains missing is measurement at industrial scale — attack throughput under sustained load, session-continuity behavior across proxy pools, and proxy market elasticity under Operator Synthesis demand. A standardized, ethical benchmark for measuring VLM-driven bypass costs — analogous to the "Bot-Bench" gap identified for VM chronometrics — is needed to transform the qualitative economic analysis of Section 5 into a quantitative discipline.
...
Academia lacks a standardized testbed for evaluating anti-automation defenses under Operator Synthesis. Current evaluations rely on grey-hat reverse engineering of production systems or small-scale PoCs that vendors invalidate through compile rotation. A reproducible, vendor-neutral evaluation harness — with known ground truth for human vs. VLM interaction — is necessary for systematic measurement. The BehavePassDB effort [23] provides a partial template but does not account for VLM interaction patterns; MCA-Bench [87] is a first step toward VLM-aware CAPTCHA benchmarking but covers only the CAPTCHA class, not VM attestation, behavioral telemetry, or attestation protocols.
...
**[7]** U. Iqbal, S. Englehardt, and Z. Shafiq. "Fingerprinting the Fingerprinters: Learning to Detect Browser Fingerprinting Behaviors." In *Proc. IEEE Symposium on Security and Privacy (S&P)*, 2021. DOI: 10.1109/SP40001.2021.00017.
...
**[10]** S. Wu, P. Sun, Y. Zhao, and Y. Cao. "Him of Many Faces: Characterizing Billion-scale Adversarial and Benign Browser Fingerprints on Commercial Websites." In *Proc. Network and Distributed System Security Symposium (NDSS)*, 2023. DOI: 10.14722/ndss.2023.24394.
...
**[20]** H. Fereidooni et al. "AuthentiSense: A Scalable Behavioral Biometrics Authentication Scheme using Few-Shot Learning for Mobile Platforms." In *Proc. Network and Distributed System Security Symposium (NDSS)*, 2023. DOI: 10.14722/ndss.2023.23194.
...
**[23]** G. Stragapede, R. Vera-Rodriguez, R. Tolosana, and A. Morales. "BehavePassDB: Public Database for Mobile Behavioral Biometrics and Benchmark Evaluation." *Pattern Recognition*, 2022. DOI: 10.1016/j.patcog.2022.109089.
...
**[76]** X. Mi, X. Feng, X. Liao, B. Liu, X. Wang, F. Qian, Z. Li, S. Alrwais, L. Sun, and Y. Liu. "Resident Evil: Understanding Residential IP Proxy as a Dark Service." In *Proc. IEEE Symposium on Security and Privacy (S&P)*, 2019. DOI: 10.1109/SP.2019.00011.
...
**[86]** X. Teoh, Y. Lin, S. Li, R. Liu, A. Sollomoni, Y. Harel, and J. S. Dong. "Are CAPTCHAs Still Bot-hard? Generalized Visual CAPTCHA Solving with Agentic Vision Language Model." In *Proc. USENIX Security Symposium*, 2025. URL: https://www.usenix.org/conference/usenixsecurity25/presentation/teoh.

**[87]** Z. Wu, Y. Xue, Y. Feng, X. Wang, and Y. Song. "MCA-Bench: A Multimodal Benchmark for Evaluating CAPTCHA Robustness Against VLM-based Attacks." arXiv:2506.05982, 2025. URL: https://arxiv.org/abs/2506.05982.

**[88]** Y. Chen, H. Zhai, C. Wang, R. Yang, L. Zhang, G. Wang, et al. "CAPTCHA Solving for Native GUI Agents: Automated Reasoning-Action Data Generation and Self-Corrective Training." arXiv:2603.23559, 2026. URL: https://arxiv.org/abs/2603.23559.
