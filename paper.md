...
| LLM tokens per action | ∼1,000 in / ∼500 out | assumption; measured 1,269–1,586 in / 24–142 out per call for two small VLMs (§5.7) |
| Actions per clearance token `n` | 5–10 | assumption; a pure-VLM agent needed 11–18 calls (median) for a five-field form (§5.7) |
...
| VLM latency per perception–action step | 5–15 s | assumption for frontier computer-use models (consistent with [98]); two small VLMs measured 0.77–1.45 s median per call (§5.7) |
...
**Detection signals follow the injection point.** The DOM-reference agent fired the honeypot in 10/10 runs. The accessibility-role agent fired it in 0/10, because the decoy is hidden from the accessibility tree. None of the 50 OS-input runs, (b)–(d), fired it. The server saw `navigator.webdriver` set in all 20 Playwright runs and in none of the 50 OS-input runs. On this testbed, both cheap in-runtime signals separate CDP automation from OS-level input and say nothing about whether a VLM is driving. This is the Axis C claim of Section 4.1. The honeypot result holds by construction: it confirms that the L3 mechanism works as described, not that it fails against any deployed system.

**Latency.** Median VLM call latency was 0.77–1.45 s for the pure agent, well below the 5–15 s per-step assumption in Table 5.1. Per-action latency for the faster model was narrow (p50 1.40 s, p90 1.89 s). This is the regular, inference-bounded distribution Section 5.2 describes, but at 1–2 s per action it overlaps plausible human form-filling times. A five-field flow took 26.3–44.1 s per run (median) for the pure agent against 7.0 s for scripted input, well inside the 1–5 minute challenge windows of Section 5.2. The 5–15 s assumption, and the L4 and session-timeout arguments built on it, therefore do not hold for fast small models. They may still hold for frontier computer-use models that reason at length, which we did not measure.

**Tokens and cost.** Input per call was 1,269–1,586 LLM tokens against the assumed ∼1,000. Output was 24–142 LLM tokens against ∼500, because the prompt asks for one JSON action and neither model emitted long reasoning text. The pure agent needed 11–18 VLM calls (median) for five fields, above the 5–10 actions assumed in Table 5.1, because it clicks, types, and submits in separate steps. Cost per success was €0.0019–€0.0136, roughly 9× to 300× below the pure-VLM row of Table 5.2 ($0.117–$0.600 at Sonnet 4.6 prices, ignoring the EUR/USD difference). The difference comes from per-token prices and a stateless prompt with no accumulated screenshots, not from fewer steps. Figures are list prices in EUR, not converted.

**Hybrid hand-back is model-dependent.** With `glm-5.3-flash` the hybrid cut VLM calls from 11 to 3.5 per run, cost per success from €0.0019 to €0.0010, and run time from 26.3 s to 15.0 s. Scripted control resumed in 9/10 runs. `qwen3.8-27b` never handed back (0/10): it ignored the instruction to stop at the first form page and filled in the whole form itself. Its hybrid row is therefore a pure-VLM run with an extra interstitial. One `qwen3.8-27b` run stopped on the first form page after filling part of it, and the flow did not complete. The low-`f` region of Table 5.2 thus assumes a hand-off that the attacker must engineer and verify per model; it does not come free with the model.

**Limits.** These numbers come from a synthetic five-field flow on localhost. N = 10 per cell, with two models from one provider, one day, and list prices. Vendor test keys always pass, so no vendor detection was measured. xdotool moves the pointer in straight jumps, which Type III kinematic scoring would likely flag, and the testbed does not score kinematics. There is no human baseline (no ethics approval for a user study), so we claim no human–VLM latency separation. The raw logs, the grounding probe, and the analysis script are in the artifact. `measurement/analyze.py --check-paper` asserts that every row of Tables 5.4 and 5.5 matches the logs.
...
**[118]** Melious. "Model hub" (per-model pages for glm-5.3-flash, qwen3.8-27b, and kimi-k2.7-code with EUR list prices per million tokens). Accessed September 23, 2026. URL: https://melious.ai/hub/models.
