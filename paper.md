...
**Scope limitation: the Advanced Persistent Bot threat model.** We draw a strict distinction between the **Commodity Bot** (basic scraper scripts, `curl`, standard headless browsers, Puppeteer) and the **Advanced Persistent Bot (APB)** (highly-resourced adversaries utilizing VLMs, custom orchestration, and industrial-scale proxy infrastructure). We acknowledge this binary elides a middle tier of moderately-resourced attackers — sophisticated Puppeteer farms with residential proxy integration and anti-detect fingerprints that operate below VLM cost thresholds — but the analysis focuses on VLM adoption as the distinguishing capability that triggers the structural re-evaluation of probabilistic defenses, not on budget alone. The probabilistic defenses of the 2010–2024 era remain effective against the overwhelming majority of web traffic (Commodity Bots) at a fraction of a cent per request. VLMs invalidate these defenses only for the top tier of motivated adversaries. Our analysis focuses on this Advanced Persistent Bot (APB) threat model, and claims about defense "collapse" should be read through this lens: a defense may be structurally bypassed for an APB while remaining economically viable against Commodity Bots.
...
## 5. An Initial Cost-Accounting Framework for VLM-Driven Attacks

*This section provides an attacker cost-accounting framework for reasoning about the economics of Operator Synthesis attacks. We do not claim to have discovered a complete microeconomic theory of VLM-based attack — the market is too young and the data too sparse, and our framework tallies costs without deriving supply/demand equilibria or utility functions. We instead provide a structured cost-accounting framework for reasoning about attacker costs, identify the empirical gaps that prevent precise quantification, and set a research agenda for closing those gaps. We approach prior work's economic framing with respect: the conjunctive cost models of Section 3 and prior SoKs [6, 24] correctly identified the multi-constraint nature of anti-automation bypass, and our contribution is to extend that framing to the Operator Synthesis paradigm, not to dismiss the foundation.*
...
1. **Chronometric heuristics (L4).** The L4 layer was originally designed to detect microsecond-scale timing deviations introduced by JavaScript instrumentation layers. Under Operator Synthesis, the instrumentation gap closes — no DevTools or debugger introduces timing noise. However, L4's detection premise is not obsolete; its *target shifts*. VLM inference introduces a *macroscopic* timing deviation: a human completes a standard VM challenge in 200–800ms of interaction time, while a VLM-driven session requires 30–120 seconds of "think time" between actions. This is not a subtle instrumentation artifact — it is a 25–50× latency gap that any chronometric heuristic can detect, because the VLM's per-action latency is bounded below by model inference speed (~5s minimum for frontier models), while human per-action latency is bounded below by reaction time (~100ms). L4 therefore survives the paradigm shift: it no longer detects *how* the browser is driven, but it detects *how fast* the operator can perceive and react. This is a qualitatively different and potentially more robust detection surface than the microsecond-precision timing analysis it replaced, because the attacker cannot close the 25–50× gap through better orchestration — they must wait for faster model inference.
...
- **Physical-presence challenges:** Defenses that require physical-world interaction (camera-based liveness detection, ambient sensor fusion) that a VLM operating in a virtual machine cannot satisfy. These are not CAPTCHAs — they do not require human cognition — but they impose a physical-presence cost that distinguishes local execution from remote VLM operation. Defenders must account for a critical countermeasure: incentivized proxying, where SDK-based proxy networks (see Section 4.3 for the structural analysis of SDK proxy attestation bypass) prompt the legitimate device owner to satisfy the physical challenge in exchange for in-app rewards, bridging the physical gap. This does not reduce the bypass cost to zero — it introduces latency (user must be available), incentive costs (per-action reward), and coordination complexity — but it establishes that physical-presence challenges require deployment-time threat modeling against SDK-mediated human relay, not just autonomous VLM operation.
