...
| LLM tokens per action | ∼1,000 in / ∼500 out | assumption; measured 613–1,586 in / 24–887 out per call for three small VLMs (§5.7) |
| Actions per clearance token `n` | 5–10 | assumption; a pure-VLM agent needed 11–18.5 calls (median) for a five-field form (§5.7) |
...
| VLM latency per perception–action step | 5–15 s | assumption for frontier computer-use models (consistent with [98]); three small VLMs measured 0.77–6.18 s median per call (§5.7) |
| Scripted-step latency | 0.5 s | assumption; xdotool input measured 0.47 s median per action (§5.7) |
...
The parameters above are assumptions. To check the token, latency, and hybrid assumptions we ran the harness of Section 7.1 against a testbed bound to localhost. The testbed flow has five pages, each with one text field and a "Continue" button, and a transparent decoy button over the real one. The decoy is the cognitive honeypot of L3 (Section 3.4): only selection by element reference (JavaScript or CDP `click()`) fires it, while a click at screen coordinates goes to the real button. Configurations: (a) Playwright over CDP, selecting elements by DOM reference (`a-dom`) or by accessibility role (`a-role`); (b) scripted xdotool keyboard input into stock Chromium with a fresh profile and no CDP; (c) a pure VLM agent that sees only a 1280×800 screenshot and acts through xdotool; (d) a hybrid that starts on a Turnstile interstitial, calls the VLM only for pages it does not recognise, and hands back to (b) once the form appears. Configurations (b)–(d) ran on a virtual X display (Xvfb). The interstitial uses Cloudflare's documented always-pass test key, so it is a single click and says nothing about vendor detection.

*Models.* We reached open and low-cost VLMs through one OpenAI-compatible provider (Melious [118], list prices in EUR). A grounding probe (25 screenshots per model, click the real "Continue") selected the models. `glm-5.3-flash` hit 25/25 at 0.71 s median, `qwen3.8-27b` 25/25 at 1.72 s, and `kimi-k2.7-code` 25/25 at 3.09 s. The open-weights `gemma-4-26b-a4b` hit 0/25 in pixel coordinates but 25/25 once its answers are read as 0–1000 normalised coordinates, so we ran it with that convention. We dropped `kimi-k2.7-code` as the slowest of the three accurate models. The agent prompt is stateless: one screenshot, the goal, and a text list of its last 12 actions per call, at temperature 0. We ran 10 runs per configuration and model on 23 September 2026. LLM spend for all runs in Table 5.5 was €0.4601.
...
Table: Table 5.4 — Detection signals and timing per configuration (N = 10 each). Pass = server-side flow completion. `navigator.webdriver` = runs whose page script reported the flag. Action latency excludes browser launch (generated by `measurement/analyze.py`).
...
Table: Table 5.5 — VLM cost per configuration (median calls per run; cost per success = total spend ÷ successes, at Melious list prices [118]).

**Detection signals follow the injection point.** The DOM-reference agent fired the honeypot in 10/10 runs. The accessibility-role agent fired it in 0/10, because the decoy is hidden from the accessibility tree. None of the 70 OS-input runs, (b)–(d), fired it. The server saw `navigator.webdriver` set in all 20 Playwright runs and in none of the 70 OS-input runs. On this testbed, both cheap in-runtime signals separate CDP automation from OS-level input and say nothing about whether a VLM is driving. This is the Axis C claim of Section 4.1. The honeypot result holds by construction: it confirms that the L3 mechanism works as described, not that it fails against any deployed system.

**Latency.** Median VLM call latency was 0.77–6.18 s for the pure agent, at or below the lower end of the 5–15 s per-step assumption in Table 5.1. Per-action latency for the fastest model was narrow (p50 1.40 s, p90 1.89 s). This is the regular, inference-bounded distribution Section 5.2 describes, but at 1–2 s per action it overlaps plausible human form-filling times. A five-field flow took 26.3–174.4 s per run (median) for the pure agent against 7.0 s for scripted input. The 5–15 s assumption, and the L4 and session-timeout arguments built on it, therefore fit slow or reasoning-heavy models better than fast small ones. For the slowest model, the 90th-percentile pure-agent run took about 380 s (6.3 min), beyond the 1–5 minute challenge windows of Section 5.2.

**Tokens and cost.** Input per call was 613–1,586 LLM tokens against the assumed ∼1,000. Output ranged from 24 to 887 LLM tokens against ∼500: it depends on whether the model emits reasoning text, not on the task. The pure agent needed 11–18.5 VLM calls (median) for five fields, above the 5–10 actions assumed in Table 5.1, because it clicks, types, and submits in separate steps. Cost per success was €0.0019–€0.0136, roughly 9× to 300× below the pure-VLM row of Table 5.2 ($0.117–$0.600 at Sonnet 4.6 prices, ignoring the EUR/USD difference). The difference comes from per-token prices and a stateless prompt with no accumulated screenshots, not from fewer steps. Figures are list prices in EUR, not converted.

**Hybrid hand-back is model-dependent.** With `glm-5.3-flash` the hybrid cut VLM calls from 11 to 3.5 per run, cost per success from €0.0019 to €0.0010, and run time from 26.3 s to 15.0 s. Scripted control resumed in 9/10 runs. `qwen3.8-27b` and `gemma-4-26b-a4b` never handed back (0/10 each): they ignored the instruction to stop at the first form page and filled in the whole form themselves. Their hybrid rows are therefore pure-VLM runs with an extra interstitial. One `qwen3.8-27b` run stopped on the first form page after filling part of it, and the flow did not complete. The low-`f` region of Table 5.2 thus assumes a hand-off that the attacker must engineer and verify per model; it does not come free with the model.

**Limits.** These numbers come from a synthetic five-field flow on localhost. N = 10 per cell, with one provider, one day, and list prices. Vendor test keys always pass, so no vendor detection was measured. xdotool moves the pointer in straight jumps, which Type III kinematic scoring would likely flag, and the testbed does not score kinematics. There is no human baseline (no ethics approval for a user study), so we claim no human–VLM latency separation. The raw logs, the grounding probe, and the analysis script are in the artifact. `measurement/analyze.py --check-paper` asserts that every row of Tables 5.4 and 5.5 matches the logs.
...
As a first step we release a self-hosted measurement harness in the artifact (`measurement/`). It consists of a testbed bound to localhost (a five-step form flow carrying a cognitive-honeypot decoy, Turnstile and reCAPTCHA v2 widgets using the vendors' documented test keys, and server-side event logging), plus four attacker configurations: DOM/CDP automation, scripted OS-level input, a pure VLM computer-use agent, and a hybrid. An analysis script turns the logs into pass rate, detection signals, per-action latency, and cost per success, pricing tokens from `analysis/params.json`. Agents are only ever pointed at this local testbed, never at third-party or production sites; test keys give fixed outcomes, so the harness measures agent cost and latency, not vendor detection. Section 5.7 reports a first small run (three VLMs, N = 10 per cell). Scaling it up — real vendor keys registered for localhost where terms allow, kinematically realistic input, a consented human baseline, and frontier computer-use models — is the next step.
...
**[118]** Melious. "Model hub" (per-model pages for glm-5.3-flash, qwen3.8-27b, gemma-4-26b-a4b, and kimi-k2.7-code with EUR list prices per million tokens). Accessed September 23, 2026. URL: https://melious.ai/hub/models.
