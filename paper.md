...
gated by `P(score below step-up threshold | spoofed layer set)` — the probability that a given evasion budget buys enough score reduction to clear the gate. This paper does not have that probability, and does not claim to. The missing empirical input is exactly the kind of gap Section 7.1 already names for the VLM-side cost model: a standardized, adversarial measurement of the score delta a given evasion budget purchases, run against a real scoring implementation and a real fingerprint-spoofing harness rather than assumed. Naming this gap sets up the relevant experiment without claiming its result.
